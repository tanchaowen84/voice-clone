...
export function testFixedWaitingLogic() {
  ...
  console.log(
    'Expected flow: Click Generate → Start waiting immediately → Hide result → Show result after wait'
  );
  ...
  console.log(
    `   - pendingAudioUrl: ${useVoiceCloneStore.getState().pendingAudioUrl ? 'STORED' : 'NULL'}`
  );
  console.log(
    `   - generatedAudioUrl: ${useVoiceCloneStore.getState().generatedAudioUrl ? 'VISIBLE' : 'HIDDEN'}`
  );
  console.log(`   - isWaiting: ${subscriptionStore.waitingState.isWaiting}`);
  console.log(
    `   - remainingTime: ${subscriptionStore.waitingState.remainingTime}s`
  );
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      console.log(
        `   - pendingAudioUrl: ${finalState.pendingAudioUrl ? 'STORED' : 'CLEARED'}`
      );
      console.log(
        `   - generatedAudioUrl: ${finalState.generatedAudioUrl ? 'VISIBLE' : 'HIDDEN'}`
      );
      console.log(
        `   - isWaiting: ${subscriptionStore.waitingState.isWaiting}`
      );
...
export function printUpdatedTestGuide() {
  ...
  console.log(
    '❌ Result should NOT appear immediately after clicking Generate'
  );
