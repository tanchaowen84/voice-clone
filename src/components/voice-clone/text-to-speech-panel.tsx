...
export function TextToSpeechPanel() {
  ...
  const showCompactWaiting = waitingState.isWaiting;
  const showPreparingState =
    isFreeUser && isGenerating && !waitingState.isWaiting;
  ...
  const requestSpeech = async (inputText: string) => {
    ...
    try {
      ...
      fetchAllData().catch(() => {
        // noop: best effort refresh for usage/plan badges
      });
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:items-start md:grid-cols-[minmax(0,1.6fr)_minmax(320px,1fr)]">
          ...
          {showPreparingState && (
            <div className="mt-4 rounded-[24px] border border-slate-200/80 bg-white/75 p-4 shadow-sm dark:border-slate-700 dark:bg-slate-950/55">
              ...
              <p className="mt-2 text-xs leading-5 text-slate-500 dark:text-slate-400">
                Your audio is being prepared now. The {freePlanWaitTime}s free
                wait starts as soon as it is ready.
              </p>
            </div>
          )}
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0">
                  ...
                  <p className="mt-2 text-sm font-semibold text-slate-900 dark:text-slate-100">
                    Skip the wait and generate instantly
                  </p>
                  <p className="mt-1 text-xs leading-5 text-slate-500 dark:text-slate-400">
                    {freePlanCharacterLimit} chars per request with a{' '}
                    {freePlanWaitTime}s wait on free. Upgrade for instant
                    generation and higher limits.
                  </p>
                </div>
